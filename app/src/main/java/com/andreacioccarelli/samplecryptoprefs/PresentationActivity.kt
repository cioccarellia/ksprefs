...
class PresentationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        prefs = CryptoPrefs(applicationContext, "Crypto", "U29maWE=")
        prefs.put("start_count", prefs.getInt("start_count", 0) + 1)
        ...
        button3.setOnClickListener {
            toast(prefs.getInt("start_count", 0))
        }
        ...
        button5.setOnClickListener {
            var x = ""
            for (pref in getSharedPreferences("Crypto", Context.MODE_PRIVATE).all) {
