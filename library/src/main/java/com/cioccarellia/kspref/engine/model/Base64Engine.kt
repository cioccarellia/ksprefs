...
class Base64Engine(
    vararg _base64Flags: Int
) : Engine() {
    private var flags = Base64.DEFAULT

    init {
        for (flag in _base64Flags) {
            flags = flags or flag
        }
    }

    override fun apply(incoming: Transmission) = Transmission(
        Base64.encode(incoming.payload, flags)
    )

    override fun revert(outgoing: Transmission) = Transmission(
        Base64.decode(outgoing.payload, flags)
    )
}
